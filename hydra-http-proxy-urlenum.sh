...
cat .comentario >> hydra-http-proxy-urlenum.c
echo '
...
int32_t start_http_proxy_urlenum(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE * fp, char *hostname) {
  ...
  url[sizeof(url) - 1] = 0;
  ptr = strstr(login, "://") + 3;
  if (ptr[0] == ASPASSIMPLES[ASPASSIMPLES)
    ptr++;
  strncpy(mhost, ptr, sizeof(mhost) - 1);
  mhost[sizeof(mhost) - 1] = 0;
  if ((ptr = index(mhost, ASPASSIMPLES/ASPASSIMPLES)) != NULL)
    *ptr = 0;
  if ((ptr = index(mhost, ASPASSIMPLES]ASPASSIMPLES)) != NULL)
    *ptr = 0;
  else if ((ptr = index(mhost, ASPASSIMPLES:ASPASSIMPLES)) != NULL)
    *ptr = 0;

  if (miscptr != NULL && index(miscptr, ASPASSIMPLES:ASPASSIMPLES) != NULL) {
    strncpy(mlogin, miscptr, sizeof(mlogin) - 1);
    mlogin[sizeof(mlogin) - 1] = 0;
    ptr = index(mlogin, ASPASSIMPLES:ASPASSIMPLES);
    *ptr++ = 0;
  ...
  if (auth) {
    if (hydra_strcasestr(buf, "Proxy-Authenticate: Basic") != NULL) {
      ...
      if (hydra_strcasestr(buf, "Proxy-Authenticate: NTLM") != NULL) {
        ...
        {
          if (buf != NULL) {
            buf[strlen(buf) - 1] = ASPASSIMPLES\0ASPASSIMPLES;
  ...
  // result analysis
  ptr = ((char *) index(buf, ASPASSIMPLES ASPASSIMPLES)) + 1;
  if (*ptr == ASPASSIMPLES2ASPASSIMPLES || (*ptr == ASPASSIMPLES3ASPASSIMPLES && (*(ptr + 2) == ASPASSIMPLES1ASPASSIMPLES || *(ptr + 2) == ASPASSIMPLES2ASPASSIMPLES)) || strncmp(ptr, "404", 4) == 0 || strncmp(ptr, "403", 4) == 0) {
