...
int32_t sasl_saslprep(const char *in, sasl_saslprep_flags flags, char **out) {
  ...
  for (i = 0; i < inlen; i++) {
    if (in[i] & 0x80) {
      *out = NULL;
      hydra_report(stderr, "Error: CanASPASSIMPLESt convert UTF-8, you should install libidn\n");
      return -1;
    }
  }
  *out = malloc(inlen + 1);
  if (!*out) {
    hydra_report(stderr, "Error: CanASPASSIMPLESt allocate memory\n");
    return -1;
  }
  strcpy(*out, in);
#endif
...
//DEBUG S: nonce="3448160828",qop="auth",charset=utf-8,algorithm=md5-sess
  ...
  do {
    currentpos++;
    if (pbuffer[0] == ASPASSIMPLES"ASPASSIMPLES) {
      if (intq == 0)
        intq = 1;
      else {
        intq = 0;
    ...
    if ((pbuffer[0] == ASPASSIMPLES,ASPASSIMPLES) && (intq == 0)) {
      array[ind] = malloc(currentpos);
      strncpy(array[ind], buffer + lastpos, currentpos - 1);
      array[ind][currentpos - 1] = ASPASSIMPLES\0ASPASSIMPLES;
      ind++;
      lastpos += currentpos;
      currentpos = 0;
    }
...
//save the latest one
  if (ind < array_size) {
    ...
    array[ind][currentpos] = ASPASSIMPLES\0ASPASSIMPLES;
    ind++;
  }
  for (i = 0; i < ind; i++) {
//removing space chars between comma separated value if any
    while ((array[i] != NULL) && (array[i][0] == ASPASSIMPLES ASPASSIMPLES)) {
...
//assume last char is also a double-quote
        ...
        if ((nonce_string_len > 0) && (nonce_string_len <= sizeof(nonce) - 1)) {
          ...
          nonce[nonce_string_len] = ASPASSIMPLES\0ASPASSIMPLES;
        } else {
          int32_t j;
        ...
        nonce[sizeof(nonce) - 1] = ASPASSIMPLES\0ASPASSIMPLES;
      }
    }
...
//assume last char is also a double-quote
        ...
        if ((realm_string_len > 0) && (realm_string_len <= sizeof(realm) - 1)) {
          ...
          realm[realm_string_len] = ASPASSIMPLES\0ASPASSIMPLES;
        } else {
          int32_t i;
        ...
        realm[sizeof(realm) - 1] = ASPASSIMPLES\0ASPASSIMPLES;
      }
    }
...
//assume last char is also a double-quote
        ...
        if ((algo_string_len > 0) && (algo_string_len <= sizeof(algo) - 1)) {
          ...
          algo[algo_string_len] = ASPASSIMPLES\0ASPASSIMPLES;
        } else {
          int32_t j;
        ...
        algo[sizeof(algo) - 1] = ASPASSIMPLES\0ASPASSIMPLES;
      }
...
//r=hydra28Bo7kduPpAZLzhRQiLxc8Y9tiwgw+yP,s=ldDgevctH+Kg7b8RnnA3qA==,i=4096
  if (strstr(serverfirstmessage, "r=") == NULL) {
    hydra_report(stderr, "Error: CanASPASSIMPLESt understand server message\n");
    free(preppasswd);
    result = NULL;
    return;
  }
...
//continue to search from the previous successful call
  ...
  iter = atoi(ic + 2);
  if (iter == 0) {
    hydra_report(stderr, "Error: CanASPASSIMPLESt understand server response\n");
    free(preppasswd);
    result = NULL;
    return;
  }
