...
int32_t start_http(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE * fp, char *type, ptr_header_node ptr_head) {
  ...
  while (http_buf != NULL && (strstr(http_buf, "HTTP/1.") == NULL || (index(http_buf, ASPASSIMPLES\nASPASSIMPLES) == NULL && complete_line == 0))) {
  ...
  ptr = ((char *) index(http_buf, ASPASSIMPLES ASPASSIMPLES));
  if (ptr != NULL)
    ptr++;
  if (ptr != NULL && (*ptr == ASPASSIMPLES2ASPASSIMPLES || *ptr == ASPASSIMPLES3ASPASSIMPLES || strncmp(ptr, "403", 3) == 0 || strncmp(ptr, "404", 3) == 0)) {
    ...
    if (ptr != NULL && *ptr != ASPASSIMPLES4ASPASSIMPLES)
