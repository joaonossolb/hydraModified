...
echo "Digite o novo User-Agent (Default: hydraModified https://github.com/joaono
ssolb/hydraModified)"
read useragent
if [ "$useragent" == "" ];then
...
' > hydra-http-proxy.c
cat .comentario >> hydra-http-proxy.c
echo '
...
int32_t start_http_proxy(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE * fp, char *hostname) {
  ...
  if (miscptr == NULL) {
    ...
    if ((ptr = index(host, ASPASSIMPLES/ASPASSIMPLES)) != NULL)
      *ptr = 0;
    if ((ptr = index(host + 6, ASPASSIMPLES:ASPASSIMPLES)) != NULL && host[0] != ASPASSIMPLES[ASPASSIMPLES)
      *ptr = 0;
    strcat(host, "\r\n");
  }
  ...
  if (http_proxy_auth_mechanism == AUTH_BASIC || hydra_strcasestr(http_proxy_buf, "Proxy-Authenticate: Basic") != NULL) {
    ...
    if (http_proxy_auth_mechanism == AUTH_NTLM || hydra_strcasestr(http_proxy_buf, "Proxy-Authenticate: NTLM") != NULL) {
      ...
      //send auth request: let the server send itASPASSIMPLESs own hostname and domainname
      buildAuthRequest((tSmbNtlmAuthRequest *) buf2, 0, NULL, NULL);
      to64frombits(buf1, buf2, SmbLength((tSmbNtlmAuthRequest *) buf2));

      /* to be portable, no snprintf, buffer is big enough so it canASPASSIMPLESt overflow */
      //send the first..
      ...
      if (pos != NULL) {
        ...
        pos += 25;
        if ((str = strchr(pos, ASPASSIMPLES\rASPASSIMPLES)) != NULL) {
          pos[str - pos] = 0;
        }
        if ((str = strchr(pos, ASPASSIMPLES\nASPASSIMPLES)) != NULL) {
          pos[str - pos] = 0;
        }
      }
      //recover challenge
      ...
      if (hydra_strcasestr(http_proxy_buf, "Proxy-Authenticate: Digest") != NULL) {
        ...
        buffer[sizeof(buffer) - 1] = ASPASSIMPLES\0ASPASSIMPLES;
        pbuffer = NULL;
      ...
      {
        if (http_proxy_buf != NULL) {
//          buf[strlen(http_proxy_buf) - 1] = ASPASSIMPLES\0ASPASSIMPLES;
  ...
  ptr = ((char *) index(http_proxy_buf, ASPASSIMPLES ASPASSIMPLES)) + 1;
  if (*ptr == ASPASSIMPLES2ASPASSIMPLES || (*ptr == ASPASSIMPLES3ASPASSIMPLES && *(ptr + 2) == ASPASSIMPLES1ASPASSIMPLES) || (*ptr == ASPASSIMPLES3ASPASSIMPLES && *(ptr + 2) == ASPASSIMPLES2ASPASSIMPLES)) {
    hydra_report_found_host(port, ip, "http-proxy", fp);
    hydra_completed_pair_found();
    free(http_proxy_buf);
    http_proxy_buf = NULL;
  } else {
    if (*ptr != ASPASSIMPLES4ASPASSIMPLES)
      hydra_report(stderr, "[INFO] Unusual return code: %c for %s:%s\n", (char) *(index(http_proxy_buf, ASPASSIMPLES ASPASSIMPLES) + 1), login, pass);
    else if (verbose && *(ptr + 2) == ASPASSIMPLES3ASPASSIMPLES)
