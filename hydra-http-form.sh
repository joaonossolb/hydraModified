...
echo "Digite o novo User-Agent (Default: hydraModified https://github.com/joaonossolb/hydraModified)"
read useragent
if [ "$useragent" == "" ];then
useragent="hydraModified https://github.com/joaonossolb/hydraModified"
fi
echo '
//Clique em Search e logo após em "Replace (ctrl+H)"
//Na primeira Caixa digite o seguinte texto: ASPASSIMPLES
' > hydra-http-form.c
cat .comentario >> hydra-http-form.c
echo '
//Marque a opção "Replace all at once"
//Clique em "Find and Replace"
...
char *strndup(const char *s, size_t n) {
  ...
  memcpy(result, s, len);
  result[len] = ASPASSIMPLES\0ASPASSIMPLES;
  return (result);
}
#endif
...
int32_t add_or_update_cookie(ptr_cookie_node * ptr_cookie, char *cookie_expr) {
  ...
  if (cookie_value) {
    ...
    // weASPASSIMPLESve got the cookieASPASSIMPLESs name and value, now itASPASSIMPLESs time to insert or update the list
    if (*ptr_cookie == NULL) {
      // no cookies
...
int32_t add_header(ptr_header_node * ptr_head, char *header, char *value, char type) {
  ...
  if (new_header && new_value) {
    if ((type == HEADER_TYPE_USERHEADER) ||
       ...
       *      2. A user-supplied header that must be appended (option ASPASSIMPLEShASPASSIMPLES).
       *      3. A user-supplied header that must replace a default header (option ASPASSIMPLEShASPASSIMPLES),
      ...
      // ItASPASSIMPLESs a user-supplied header that must replace a default one
      // Replace the default headerASPASSIMPLESs value with this new value
    ...
    // weASPASSIMPLESre out of memory, so forcefully end
    free(new_header);
    free(new_value);
    return 0;
  }
...
/*
 * Replace in all headersASPASSIMPLES values every occurrence of oldvalue by newvalue.
...
/*
 * Replace the value of the default header named ASPASSIMPLEShdrnameASPASSIMPLES.
 */
...
int32_t parse_options(char *miscptr, ptr_header_node *ptr_head) {
  ...
  while (*miscptr != 0) {
    switch (miscptr[0]) {
    case ASPASSIMPLEScASPASSIMPLES:                  // fall through
    case ASPASSIMPLESCASPASSIMPLES:
      ptr = miscptr + 2;
      while (*ptr != 0 && (*ptr != ASPASSIMPLES:ASPASSIMPLES || *(ptr - 1) == ASPASSIMPLES\\ASPASSIMPLES))
        ptr++;
      if (*ptr != 0)
        *ptr++ = 0;
      ...
      miscptr = ptr;
      break;
    case ASPASSIMPLEShASPASSIMPLES:
      // add a new header at the end
      ptr = miscptr + 2;
      while (*ptr != 0 && *ptr != ASPASSIMPLES:ASPASSIMPLES)
        ptr++;
      if (*(ptr - 1) == ASPASSIMPLES\\ASPASSIMPLES)
        *(ptr - 1) = 0;
      if (*ptr != 0) {
        *ptr = 0;
        ptr += 2;
      ...
      ptr2 = ptr;
      while (*ptr2 != 0 && (*ptr2 != ASPASSIMPLES:ASPASSIMPLES || *(ptr2 - 1) == ASPASSIMPLES\\ASPASSIMPLES))
        ptr2++;
      if (*ptr2 != 0)
        *ptr2++ = 0;
      /*
       * At this point:
       *  - (optional1 + 2) contains the headerASPASSIMPLESs name
       *  - ptr contains the headerASPASSIMPLESs value
       */
      ...
      return 0;
    case ASPASSIMPLESHASPASSIMPLES:
      // add a new header, or replace an existing oneASPASSIMPLESs value
      ptr = miscptr + 2;
      while (*ptr != 0 && *ptr != ASPASSIMPLES:ASPASSIMPLES)
        ptr++;
...
char *html_encode(char *string) {
  ...
  if (index(ret, ASPASSIMPLES%ASPASSIMPLES) != NULL)
...
int32_t analyze_server_response(int32_t s) {
  ...
  while ((buf = hydra_receive_line(s)) != NULL) {
    ...
    if (hydra_strcasestr(buf, "Location: ") != NULL) {
      ...
      str[sizeof(str) - 1] = 0;
      endloc = strchr(str, ASPASSIMPLES\nASPASSIMPLES);
      if (endloc != NULL)
        *endloc = 0;
      endloc = strchr(str, ASPASSIMPLES\rASPASSIMPLES);
      if (endloc != NULL)
        *endloc = 0;
    ...
    if (hydra_strcasestr(buf, "Set-Cookie: ") != NULL) {
      ...
      do {
        ...
        str[sizeof(str) - 1] = 0;
        endcookie1 = strchr(str, ASPASSIMPLES\nASPASSIMPLES);
        endcookie2 = strchr(str, ASPASSIMPLES;ASPASSIMPLES);
        //terminate string after cookie data
        if (endcookie1 != NULL && ((endcookie1 < endcookie2) || (endcookie2 == NULL))) {
          if (*(endcookie1 - 1) == ASPASSIMPLES\rASPASSIMPLES)
            endcookie1--;
          *endcookie1 = 0;
        } else if (endcookie2 != NULL)
          *endcookie2 = 0;
        // is the cookie already there? if yes, remove it!
        if (index(startcookie, ASPASSIMPLES=ASPASSIMPLES) != NULL && (ptr = index(startcookie, ASPASSIMPLES=ASPASSIMPLES)) - startcookie + 1 <= sizeof(tmpname)) {
          strncpy(tmpname, startcookie, sizeof(tmpname) - 2);
          tmpname[sizeof(tmpname) - 2] = 0;
          ptr = index(tmpname, ASPASSIMPLES=ASPASSIMPLES);
          *(++ptr) = 0;
          ...
          if ((ptr = hydra_strcasestr(cookie, tmpname)) != NULL) {
            ...
            if (ptr != cookie && *(ptr - 1) == ASPASSIMPLES ASPASSIMPLES) {
        ...
        ptr = index(str, ASPASSIMPLES=ASPASSIMPLES);
        // only copy the cookie if it has a value (otherwise the server wants to delete the cookie)
        if (ptr != NULL && *(ptr + 1) != ASPASSIMPLES;ASPASSIMPLES && *(ptr + 1) != 0 && *(ptr + 1) != ASPASSIMPLES\nASPASSIMPLES && *(ptr + 1) != ASPASSIMPLES\rASPASSIMPLES) {
          if (strlen(cookie) > 0)
...
int32_t start_http_form(int32_t s, char *ip, int32_t port, unsigned char options, char *miscptr, FILE * fp, char *hostname, char *type, ptr_header_node ptr_head,
  ...
  /* again: no snprintf to be portable. donASPASSIMPLESt worry, buffer canASPASSIMPLESt overflow */
  ...
  while (found == 0 && redirected_flag && (redirected_url_buff[0] != 0) && (redirected_cpt > 0)) {
    ...
    if (strstr(redirected_url_buff, cond) != NULL) {
      ...
      if (startloc != NULL) {
        ...
        if ((endloc = strchr(startloc, ASPASSIMPLES\rASPASSIMPLES)) != NULL) {
          startloc[endloc - startloc] = 0;
        }
        if ((endloc = strchr(startloc, ASPASSIMPLES\nASPASSIMPLES)) != NULL) {
          startloc[endloc - startloc] = 0;
        }
        strcpy(str, startloc);

        endloc = strchr(str, ASPASSIMPLES/ASPASSIMPLES);
        if (endloc != NULL) {
        ...
        if (redirected_url_buff[0] != ASPASSIMPLES/ASPASSIMPLES) {
          //itASPASSIMPLESs a relative path, so we have to concatenate it
          ...
          urlpath = strrchr(url, ASPASSIMPLES/ASPASSIMPLES);
          if (urlpath != NULL) {
      ...
      if (str3[0] != ASPASSIMPLES/ASPASSIMPLES) {
        j = strlen(str3);
        str3[j + 1] = 0;
        for (i = j; i > 0; i--)
          str3[i] = str3[i - 1];
        str3[0] = ASPASSIMPLES/ASPASSIMPLES;
      }

      if (strrchr(url, ASPASSIMPLES:ASPASSIMPLES) == NULL && port != 80) {
...
ptr_header_node initialize(char *ip, unsigned char options, char *miscptr) {
  ...
  if (webtarget != NULL && (webtarget = strstr(miscptr, "://")) != NULL) {
    webtarget += strlen("://");
    if ((ptr2 = index(webtarget, ASPASSIMPLES:ASPASSIMPLES)) != NULL) {       /* step over port if present */
      *ptr2 = 0;
      ptr2++;
      ptr = ptr2;
      if (*ptr == ASPASSIMPLES/ASPASSIMPLES || (ptr = index(ptr2, ASPASSIMPLES/ASPASSIMPLES)) != NULL)
        miscptr = ptr;
      else
        miscptr = slash;        /* to make things easier to user */
    } else if ((ptr2 = index(webtarget, ASPASSIMPLES/ASPASSIMPLES)) != NULL) {
  ...
  if (use_proxy == 1 && proxy_authentication[selected_proxy] != NULL) {
    ...
    add_header(&ptr_head, "User-Agent", "'$useragent'", HEADER_TYPE_DEFAULT);
...
void usage_http_form(const char *service) {
  printf("Module %s requires the page and the parameters for the web form.\n\n"
         ...
         "                 Note: ASPASSIMPLEShASPASSIMPLES will add the user-defined header at the end\n"
         "                 regardless itASPASSIMPLESs already being sent by Hydra or not.\n"
         "                 ASPASSIMPLESHASPASSIMPLES will replace the value of that header if it exists, by the\n"
